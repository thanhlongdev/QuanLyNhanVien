...
<body>
	<div class="container-fluid d-flex justify-content-center">
		<div
			...
			<form method="post" action="admin/record/add-record.htm"
				...
				<div class="form-group my-3 text-center" data-plugin="formMaterial">
					<h3><s:message code="admin.record.form.addtitle"/></h3>
				</div>
				<div class="form-group">
					<label for="exampleFormControlSelect1">
						<s:message code="admin.record.table.col5"/>
					</label> <select
						class="form-control" name="staffId" id="combobox">
						<c:forEach var="staff" items="${listStaff}">
							<option value="${staff.id.trim()}">${staff.id.trim()}-
								${staff.name}</option>
						</c:forEach>
					</select>
				</div>
				...
				<div class="form-group my-3" data-plugin="formMaterial">
					...
					<div class="form-check-inline">
						<label class="form-check-label" for="radio2"> <input
							...
							value="true" checked>Khen Thưởng (Prise)
						</label>
					</div>
					<div class="form-check-inline">
						<label class="form-check-label" for="radio2"> <input
							type="radio" class="form-check-input" id="radio2" name="type"
							value="false">Kỷ Luật (Discipline)
						</label>
					</div>
				</div>
				...
				<div class="form-group my-3" data-plugin="formMaterial">
					<label class="form-control-label" for="inputPassword">
						...
						id="datePicker" name="date" placeholder="Text" readonly required>
				</div>
				...
				<button class="btn btn-primary mb-3 float-right" type="submit">
					<s:message code="admin.record.form.addtitle"/>
				</button>
			</form>
		</div>
	</div>
